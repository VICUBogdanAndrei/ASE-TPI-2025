function []=ex_Fourier(nume, tip)
    ...
    poza=imread(nume);
    [m,n,p]=size(poza);
    % conversie la monocrom, daca e nevoie. 
    % exemplul lucreaza pe imagini monocrome
    if p>1
        plan=rgb2gray(poza);
    else
        plan=poza;
    ...
    Spectru=abs(y); % spectrul imaginii (raza, magnitudinea)
    %   atan2 calc. arctangenta in [-pi,pi]; atan calc. in [-pi/2,pi/2]
    Faza=atan2(I,R);% faza imaginii (unghi)
    
    % reprezentare polara
    r_polara=zeros(m,n);
    for i=1:m
        for j=1:n
            % 1i reprezinta partea imaginara (i din matematica)
            r_polara(i,j)= Spectru(i,j) * exp( 1i*Faza(i,j) );
        end;
    end;
    % y si r_polara sint aproximativ egale (doar diferente minuscule date
    % de erorile de calcule si reprezentare)
    % pentru verificare
    % dif=y-r_polara;
    % find(dif>0.00001); % rezulta matrice goala
    % find(dif<0.00001); % rezulta matrice goala
    ...
    % imagine reconstruita folosind doar spectrul
    g2=ifft2(Spectru);
    %g2=centrare(g2);
    figure
        imshow(uint8(g2));
        title('Imaginea reconstruita folosind doar spectrul');
    fi=[nume '-spectru.' tip];
